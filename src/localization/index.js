...
class LocalizationManager {
  ...
  updateUI() {
    ...
    qrnLabels.forEach((label) => {
      if (
        label.textContent.includes('QRN (Atmospheric Noise)') ||
        label.textContent.includes('QRN (Атмосферные помехи)')
      ) {
    ...
    callsignFormatLabels.forEach((label) => {
      if (
        label.textContent.includes('Callsign Format Options') ||
        label.textContent.includes('Формат позывных') ||
        label.textContent.includes('Опции формата позывных')
      ) {
    ...
    if (tableHeaders.length > 0) {
      ...
      if (tableHeaders[5])
        tableHeaders[5].textContent = this.t('results.additionalInfo');
    }
